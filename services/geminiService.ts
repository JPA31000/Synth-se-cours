...
export const generateLogoImage = async (): Promise<string> => {
    if (!process.env.API_KEY) {
        throw new Error("API_KEY environment variable is not set.");
    }
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    ...
    try {
        const response = await ai.models.generateImages({
            ...
            prompt: 'A modern, minimalist logo for an AI-powered study tool. The logo features a stylized book with a glowing spark above it, representing knowledge and insight. Use a vibrant color palette of lime green (#84cc16) and dark slate gray (#334155). The style is clean, flat, and professional, on a white background.',
            config: {
                numberOfImages: 1,
        ...
        if (response.generatedImages && response.generatedImages.length > 0) {
            ...
            throw new Error("No logo image was generated by the API.");
        }
    } catch (error) {
        console.error("Error generating logo image:", error);
        throw new Error("Failed to generate logo image from Gemini API.");
    }
};
